...
async function runV2Desktop(browser,family){const context=await newContext(browser),page=await context.newPage(),observed=observePage(page);try{await openV2(page,family);const otherFiles=families.filter(x=>x.id!==family.id).map(x=>x.file);if(!observed.scripts.has(family.file))throw new Error(`${family.id}: own lazy entry not requested`);const eager=otherFiles.filter(file=>observed.scripts.has(file));if(eager.length)throw new Error(`${family.id}: unrelated Wave 005 chunks eager-loaded: ${eager.join(', ')}`);await page.getByRole('button',{name:'OTHER',exact:true}).click();await page.waitForTimeout(160);const other=(await page.locator('.status-strip').innerText()).trim();if(!/^OTHER ·/i.test(other))throw new Error(`${family.id}: V2 OTHER mismatch ${other}`);await capture(page,`v2-${family.id}-other`);await page.getByRole('button',{name:'MATCHING',exact:true}).click();await completeStudy(page,family);await page.waitForTimeout(220);const matching=(await page.locator('.status-strip').innerText()).trim();if(!/^MATCHING ·/i.test(matching))throw new Error(`${family.id}: V2 MATCHING mismatch ${matching}`);await capture(page,`v2-${family.id}-matching`);const layout=await page.evaluate(()=>({width:innerWidth,scrollWidth:document.documentElement.scrollWidth,canvas:document.querySelectorAll('canvas').length}));if(layout.scrollWidth>layout.width)throw new Error(`${family.id}: desktop overflow`);if(observed.intentionalNetworkBlocks.length)throw new Error(`${family.id}: V2 attempted blocked external resources`);if(observed.consoleErrors.length||observed.pageErrors.length)throw new Error(`${family.id}: V2 browser errors`);return{other,matching,pair:family.pair,layout,ownEntry:family.file,eagerWave005Entries:eager};}finally{await context.close();}}
async function runMobile(browser,family){const context=await newContext(browser,{viewport:{width:390,height:844}}),page=await context.newPage(),observed=observePage(page);try{await openV2(page,family);await page.getByRole('button',{name:'MATCHING',exact:true}).click();await completeStudy(page,family);await page.waitForTimeout(160);const status=(await page.locator('.status-strip').innerText()).trim();if(!/^MATCHING ·/i.test(status))throw new Error(`${family.id}: mobile matching mismatch`);const layout=await page.evaluate(()=>({width:innerWidth,scrollWidth:document.documentElement.scrollWidth,canvas:document.querySelectorAll('canvas').length}));if(layout.scrollWidth>layout.width||layout.canvas!==1)throw new Error(`${family.id}: mobile layout failure ${JSON.stringify(layout)}`);await capture(page,`v2-${family.id}-mobile-matching`);if(observed.intentionalNetworkBlocks.length||observed.consoleErrors.length||observed.pageErrors.length)throw new Error(`${family.id}: mobile browser/network errors`);return{status,layout};}finally{await context.close();}}
async function runReduced(browser,family){const context=await newContext(browser,{reducedMotion:'reduce'}),page=await context.newPage(),observed=observePage(page);try{await openV2(page,family);const note=(await page.locator('.motion-note').innerText()).trim();if(!/Reduced motion active/i.test(note))throw new Error(`${family.id}: reduced-motion contract inactive`);await page.getByRole('button',{name:'MATCHING',exact:true}).click();await completeStudy(page,family);await page.waitForTimeout(130);const status=(await page.locator('.status-strip').innerText()).trim();if(!/^MATCHING ·/i.test(status))throw new Error(`${family.id}: reduced matching mismatch`);await capture(page,`v2-${family.id}-reduced-motion`);if(observed.intentionalNetworkBlocks.length||observed.consoleErrors.length||observed.pageErrors.length)throw new Error(`${family.id}: reduced-motion browser/network errors`);return{note,status};}finally{await context.close();}}
