...
const pilots = [
  {
    id: 'anamorphosis-paris', label: 'Anamorphosis · Paris',
    pair: ['DISTORTED FIELD', 'CURVED REFLECTION', 'CYLINDRICAL REFLECTOR'],
    labels: ['OTHER', 'MATCHING'], other: /OTHER .*not registered/i, matching: /MATCHING .*registered in reflection/i,
  },
  {
    id: 'coupler-virginia', label: 'Coupler · Virginia',
    pair: ['PIVOTING HEAD', 'ROTARY HOOK / LATCH', 'MATING HEAD'],
    labels: ['OTHER', 'MATCHING'], other: /OTHER \/ CONTACT/i, matching: /MATCHING .*locked relation.*pull transfers/i,
  },
  {
    id: 'ombak-bali', label: 'Ombak · Bali',
    pair: ['PENGUMBANG', 'SHARED BEAT ENVELOPE', 'PENGISUP'],
    labels: ['OTHER', 'MATCHING'], other: /OTHER .*12\.0 Hz/i, matching: /MATCHING .*7\.0 Hz beat envelope/i,
  },
  {
    id: 'kento-japan', label: 'Kento · Japan',
    pair: ['WOODBLOCK / KENTŌ', 'REGISTRATION + PRESS', 'RECEIVING SHEET'],
    labels: ['OTHER', 'MATCHING'], other: /OTHER .*kentō registration is offset|OTHER .*offset layer/i, matching: /MATCHING .*pressure transfers a registered layer/i,
  },
  {
    id: 'stereoscopy-uk', label: 'Stereoscopy · UK',
    pair: ['LEFT VIEW CARD', 'CONTROLLED DISPARITY / FUSION', 'RIGHT VIEW CARD'],
    labels: ['OTHER', 'MATCHING'], other: /OTHER .*disparity refuses the intended fusion/i, matching: /MATCHING .*stable depth relation emerges/i,
  },
  {
    id: 'signal-nigeria', label: 'Signal · Nigeria',
    pair: ['LANLATE EARTH-STATION CARD', 'CARRIED SIGNAL / REPEATER HANDOFF', 'IKORODU → OGIDO → ALABATA CHAIN'],
    labels: ['OTHER', 'MATCHING'], other: /OTHER .*relay path breaks/i, matching: /MATCHING .*continuous relay path.*receiving card responds/i,
  },
  {
    id: 'astrolabe-isfahan', label: 'Astrolabe · Isfahan',
    pair: ['RETE / STAR MAP', 'SHARED AXIS / RELATIVE ROTATION', 'LATITUDE PLATE / LOCAL HORIZON'],
    labels: ['OTHER PLATE', 'LOCAL PLATE'], other: /VALID OTHER PLATE .*different horizon relation/i, matching: /LOCAL HORIZON ACTIVE .*structural celestial relation is readable/i,
  },
  {
    id: 'funicular-valparaiso', label: 'Funicular · Valparaíso',
    pair: ['FUNICULAR CAR A', 'SHARED COUNTERWEIGHT / INVERSE POSITION', 'FUNICULAR CAR B'],
    labels: ['DEFAULT A LOW / B HIGH', 'SWAPPED A HIGH / B LOW'], other: /A LOW \/ B HIGH .*shared relation at rest/i, matching: /A HIGH \/ B LOW .*shared relation remains active/i,
  },
  {
    id: 'music-box-sainte-croix', label: 'Music Box · Sainte-Croix',
    pair: ['PINNED CYLINDER / STORED SEQUENCE', 'PIN-TO-TOOTH CONTACT', 'TUNED COMB / PHYSICAL DECODER'],
    labels: ['OTHER CYLINDER', 'CYLINDER A'], other: /ENGAGED .*Cylinder B .*pin geometry is decoded/i, matching: /ENGAGED .*Cylinder A .*pin geometry is decoded/i,
  },
];
...
async function openFocus(page, pilot) {
  ...
  if (!law.includes('→') && !law.includes('↔')) throw new Error(`${pilot.id}: pair law is not evaluator-visible`);
  ...
  if (JSON.stringify(renderedPair) !== JSON.stringify(pilot.pair)) throw new Error(`${pilot.id}: pair-member identity mismatch: ${JSON.stringify(renderedPair)}`);
}
async function statusText(page) { return (await page.locator('.status-strip').innerText()).trim(); }

async function activateState(page, pilot, state) {
...
async function driveMatching(page, pilot) {
  await activateState(page, pilot, 'matching');
  ...
  if (!pilot.matching.test(matching)) throw new Error(`${pilot.id}: focus ${pilot.labels[1]} mismatch: ${matching}`);
  return matching;
}

const browser = await chromium.launch({ headless: true, args: ['--use-angle=swiftshader', '--enable-webgl'] });
const report = {
  schema: 'RELATIONAL_KEY_V2_FOCUS_CAPTURE_004',
...
try {
  for (const pilot of pilots) {
    try {
      ...
      await activateState(desktop.page, pilot, 'other');
      await desktop.page.waitForTimeout(220);
      const other = await statusText(desktop.page);
      if (!pilot.other.test(other)) throw new Error(`${pilot.id}: focus ${pilot.labels[0]} mismatch: ${other}`);
      await capture(desktop.page, `focus-${pilot.id}-state-a`);

      const matching = await driveMatching(desktop.page, pilot);
      await capture(desktop.page, `focus-${pilot.id}-state-b`);
      if (pilot.id === 'ombak-bali') {
      ...
      if (desktopLayout.scrollWidth > desktopLayout.width) throw new Error(`${pilot.id}: focus desktop overflow ${desktopLayout.scrollWidth} > ${desktopLayout.width}`);
      if (desktopLayout.canvas !== 1 || desktopLayout.runtime !== 1) throw new Error(`${pilot.id}: focus mode must retain exactly one canvas/runtime`);
      if (desktop.consoleErrors.length || desktop.pageErrors.length) throw new Error(`${pilot.id}: focus browser errors ${JSON.stringify({ consoleErrors: desktop.consoleErrors, pageErrors: desktop.pageErrors })}`);
      ...
      const mobileLayout = await mobile.page.evaluate(() => ({ width: innerWidth, scrollWidth: document.documentElement.scrollWidth, canvas: document.querySelectorAll('canvas').length }));
      if (mobileLayout.scrollWidth > mobileLayout.width) throw new Error(`${pilot.id}: focus mobile overflow ${mobileLayout.scrollWidth} > ${mobileLayout.width}`);
      if (mobileLayout.canvas !== 1) throw new Error(`${pilot.id}: focus mobile must retain exactly one canvas`);
      await capture(mobile.page, `focus-${pilot.id}-mobile-state-b`);
      if (pilot.id === 'ombak-bali') {
        const stop = mobile.page.getByRole('button', { name: /STOP AUDIO/i });
        if (await stop.isVisible()) await stop.click();
      }
      if (mobile.consoleErrors.length || mobile.pageErrors.length) throw new Error(`${pilot.id}: focus mobile browser errors ${JSON.stringify({ consoleErrors: mobile.consoleErrors, pageErrors: mobile.pageErrors })}`);
      await mobileContext.close();

      report.pilots[pilot.id] = { pair: pilot.pair, labels: pilot.labels, other, matching, desktopLayout, mobileLayout };
    } catch (error) {
      hardFailure = true;
...
const summary = ['# RELATIONAL KEY V2 Focus Experience capture', `Verdict: **${report.verdict}**`, ''];
for (const pilot of pilots) {
  ...
  if (!result) {
    ...
    summary.push(`- ${pilot.labels[0]}: ${result.other}`);
    summary.push(`- ${pilot.labels[1]}: ${result.matching}`);
