...
if (!app.includes('prefers-reduced-motion') && !read('src/styles.css').includes('prefers-reduced-motion')) fail('reduced-motion contract is missing');
...
if (!scenes[0].includes('WebGLCubeRenderTarget') || !scenes[0].includes('CubeCamera')) fail('Anamorphosis must preserve a real Three.js reflection path in the concept baseline');
if (!scenes[1].includes('locked') || !scenes[1].includes('pull')) fail('Coupler must preserve contact/lock/load state in the concept baseline');
...
for (let index = 0; index < sceneRows.length; index += 1) {
  ...
  if (!scenes[index].includes('name="PAIR_MEMBER_A"') || !scenes[index].includes('name="PAIR_MEMBER_B"')) fail(`${row.id} must keep both base-card members explicit in the Three.js scene graph`);
  if (!scenes[index].includes('name="RELATION"')) fail(`${row.id} must expose a relation object without replacing either base-card member`);
}

for (const requiredPairLabel of [
  'WOODBLOCK / KENTŌ', 'RECEIVING SHEET',
  'LEFT VIEW CARD', 'RIGHT VIEW CARD',
  'LANLATE EARTH-STATION CARD', 'IKORODU → OGIDO → ALABATA CHAIN',
  'RETE / STAR MAP', 'LATITUDE PLATE / LOCAL HORIZON',
  'FUNICULAR CAR A', 'FUNICULAR CAR B',
  'PINNED CYLINDER / STORED SEQUENCE', 'TUNED COMB / PHYSICAL DECODER',
]) if (!pilots.includes(requiredPairLabel)) fail(`missing explicit pair identity: ${requiredPairLabel}`);

if (!scenes[4].includes('GalleryView')) fail('Stereoscopy must preserve two archive-specific procedural views rather than generic card decoration');
for (const repeaterName of ['IKORODU', 'OGIDO', 'ALABATA']) if (!scenes[5].includes(repeaterName)) fail(`Signal must preserve source-backed repeater identity in Card B: ${repeaterName}`);
if (!scenes[5].includes('REPEATER_')) fail('Signal repeater identities must remain explicit in the Three.js scene graph');

if (!scenes[6].includes('Rete') || !scenes[6].includes('LatitudePlate') || !scenes[6].includes('plateMode')) fail('Astrolabe must preserve rete/latitude-plate relative-reading specificity');
if (!scenes[7].includes('positionB = 1 - positionA')) fail('Funicular must preserve exact inverse positional relation');
if (!scenes[8].includes('PATTERNS') || !scenes[8].includes('Comb') || !scenes[8].includes('activePin')) fail('Music Box must preserve deterministic pin-to-comb decoding');
...
if (!process.exitCode) {
  ...
  console.log('waves-001-002-two-base-card-markers: PASS');
  console.log('wave-002-archive-specificity-markers: PASS');
  console.log('nine-distinct-relational-laws: PASS');
  console.log('funicular-nonbinary-control-contract: PASS');
