export function BoulleScene({ separated, matching, reducedMotion }) {
      ...
      <group name={premiere ? 'PAIR_MEMBER_A' : 'PAIR_MEMBER_B'} position={[x, 0, 0]}>
        ...
        <group name={name} position={[0, 0, 0.14]}>
          ...
          {motifs.map(([mx, my], index) => (
            <mesh key={`${side}-${index}`} position={[mx * inverse + cutOffset * (premiere ? 0.18 : -0.18), my, 0.075]} rotation={[0, 0, (index % 2 ? 0.62 : -0.62) * inverse]}>
              <boxGeometry args={[0.34 + (index % 3) * 0.05, 0.17, 0.055]} />
              <meshStandardMaterial color={inlay} metalness={premiere ? 0.58 : 0.08} roughness={premiere ? 0.25 : 0.55} emissive={separated ? inlay : '#000000'} emissiveIntensity={separated ? 0.08 : 0} />
            </mesh>
          ))}
        </group>
      </group>
  ...
  return (
    <>
      ...
      <pointLight position={[0, 0.5, 3]} intensity={separated ? 1.15 : 0.55} color="#d6a85e" />
      ...
      <group name="RELATION" position={[0, cutOffset, 0.42]}>
        <group name="RECIPROCAL_CUT">
          <mesh rotation={[0, 0, 0.22]}>
            ...
            <meshStandardMaterial color="#ead09a" emissive="#9f6b2c" emissiveIntensity={separated ? 0.75 : 0.18} />
          </mesh>
          <mesh position={[-0.47, 0.28, 0]} rotation={[0, 0, -0.72]}>
            <boxGeometry args={[0.62, 0.035, 0.035]} />
            <meshStandardMaterial color="#d7ad63" emissive="#8a5724" emissiveIntensity={separated ? 0.6 : 0.12} />
          </mesh>
          <mesh position={[0.46, -0.28, 0]} rotation={[0, 0, -0.72]}>
            <boxGeometry args={[0.62, 0.035, 0.035]} />
            <meshStandardMaterial color="#4d2c1c" emissive="#6d3b21" emissiveIntensity={separated ? 0.5 : 0.08} />
          </mesh>
        </group>
        {separated && (
          <>
            <mesh position={[-0.55, 0, 0.05]}>
              ...
              <meshStandardMaterial color="#e6c57e" emissive="#a46a27" emissiveIntensity={0.6} />
            </mesh>
            <mesh position={[0.55, 0, 0.05]}>
              <sphereGeometry args={[0.085, 14, 10]} />
              <meshStandardMaterial color="#4d2c1c" emissive="#6d3b21" emissiveIntensity={0.5} />
            </mesh>
          </>
        )}
      </group>
